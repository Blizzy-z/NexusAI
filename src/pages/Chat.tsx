...
import { getGeminiResponse, getOllamaResponse, GEMINI_TOOLS } from '../services/api';
...
export default function Chat() {
  ...
  const createNewSession = (projectId?: string) => {
    const newSession: ChatSession = {
      ...
      messages: [],
      projectId,
      systemPrompt: "You are NexusAI. Be direct and casual. Keep replies SHORT -- 1-3 sentences max unless the user asks for more. Don't volunteer extra info or ask follow-up questions. Just answer what was asked, nothing else. No disclaimers."
    };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (selectedModel.startsWith('gemini')) {
        ...
        const r = await getGeminiResponse(input, systemPrompt, selectedModel, tools.length ? tools : undefined) as any;
        ...
        // #endregion
        const text = await getOllamaResponse(input, selectedModel, systemPrompt);
